...
This is the Beta version of the store used for testing. DO NOT USE THIS VERSION. . 


== Description ==

This is the Beta version of the store used for testing. DO NOT USE THIS VERSION. WP Online Store is a all in one eCommerce plugin for WordPress that utilizes the legendary osCommerce online store software. Hundreds of  easy to use features, integrates into ANY WordPress theme + great support. 
WP Online Store with osCommerce is one of the most popular eCommerce solutions on the internet and has more features than we could ever list (we tried below). It also has a very easy to use admin area, so set up is a breeze. Having your WordPress site become a fully operational online store has never been easier, its as simple as 1- Install, 2- Activate, and 3- Add store to page.
...
= WP Online Store 1.3.1 Features: = (Uses osCommerce V.2.3.1)

*  NEW: Grid or list view option for product pages (selectable in admin area or switchable on front end by user).
*  NEW: Product sort order for visitors. Now users can sort products by price, most popular, newest, most viewed.
*  NEW: Order editor
*  NEW: Login Radius social media login system
*  NEW: "off" switch for Google translate
*  NEW: Product sort order
*  NEW: Added a Product qty field to the product page.
*  NEW: Added "Continue Shopping" button to checkout.
*  NEW: Added "next" and "previous" buttons to product pages.
*  NEW: Fixed small search issue (search was only checking product names not the descriptions)
*  NEW: Took out HTML coding from product titles to avoid special character issues.
*  Ajax attribute manager. All features enabled by default for a complete ready-to-go solution.
...
*  Multiple shipping calculators available (FedEx, UPS USPS etc) plus the ability to add additional worldwide shipping companies.
...
*  Inventory tracking (site warns you when your inventory is low, can even display "out of stock" message if you run out).
...
*  Spanish language pack available (other additional languages being worked on).
...
*  Weight, price, and destination based shipping modules available
