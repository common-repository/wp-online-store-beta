...
// Text Mode
  ...
  function tep_get_products_name($product_id, $language_id = 0) {
    ...
    return stripslashes($product['products_name']);
  }
  ...
  function tep_get_products_description($product_id, $language_id) {
    ...
    return stripslashes($product['products_description']);
  }
...
// The $wrap parameter is no longer used in the core xhtml template
  function tep_draw_fck_field($name, $wrap, $width, $height, $text = '',  $reinsert_value = true) {
...
// seo url support 
  ...
  function tep_get_products_seo_url($product_id) {
    $product_query = tep_db_query("select products_seo_url from " . TABLE_PRODUCTS . " where products_id = '" . (int)$product_id . "'");
    $product = tep_db_fetch_array($product_query);
