...
// The HTML href link wrapper function
  function tep_href_link($page = '', $parameters = '', $connection = 'NONSSL') {
    ...
    if ($parameters == '') {
      $link = $link .'admin.php?page=WP_online_store&submenu='. $page . '&' . SID;
    } else {
      $link = $link .'admin.php?page=WP_online_store&submenu='. $page . '&' . tep_output_string($parameters) . '&' . SID;
    }
...
function tep_mail_catalog_href_link($page='',$parameters='',$connection = 'NONSSL'){
global $wpdb;
...
foreach($posts as $post)
    	{
    		$pos = strpos($post->post_content, "[WP_online_store]");
    		if($pos!==false)
    			$link=$post->guid;
    	}
...
// The HTML image wrapper function
  ...
  $tsrc=substr($src,0,4);
  if(strtolower($tsrc)=="http")
  	$image = '<img src="' . tep_output_string($src) . '" border="0" alt="' . tep_output_string($alt) . '"';
  else
    $image = '<img src="' . tep_output_string(HTTP_CATALOG_SERVER.DIR_WS_CATALOG.'admin/'.$src) . '" border="0" alt="' . tep_output_string($alt) . '"';
...
// Outputs a button in the selected language
  function tep_image_submit($image, $alt = '', $parameters = '') {
    ...
    $image_submit = '<input type="image" src="' . tep_output_string(HTTP_CATALOG_SERVER.DIR_WS_CATALOG.'admin/includes/'.DIR_WS_LANGUAGES . $language . '/images/buttons/' . $image) . '" border="0" alt="' . tep_output_string($alt) . '"';
...
// Output a separator either through whitespace, or with an image
  function tep_draw_separator($image = 'pixel_black.gif', $width = '100%', $height = '1') {
    return tep_image(HTTP_CATALOG_SERVER.DIR_WS_CATALOG.'admin/'.DIR_WS_IMAGES . $image, '', $width, $height);
  }
