...
*/
$paypal_standard_code = 'All right reserved by IMC group';
/*
...
// EOF - Zappo - Option Types v2 - Correction for Text Option Values
    ...
    function process_button() {
      global $customer_id, $order, $sendto, $currency, $cart_PayPal_Standard_ID, $shipping,$discount;
      ...
      $parameters = array('cmd' => '_xclick',
                          ...
                          'amount' => $this->format_raw($order->info['total'] - $order->info['shipping_cost'] - $order->info['tax']+ (empty($discount) ? 0 : $discount)),
...
//------insert customer choosen option to order--------
        ...
        if (isset($order->products[$i]['attributes'])) {
          ...
          for ($j=0, $n2=sizeof($order->products[$i]['attributes']); $j<$n2; $j++) {
            ...
            $attributes_values = tep_db_fetch_array($attributes);
// BOF - Zappo - Option Types v2 - Adjustment for Text Option Types
            $products_ordered_attributes .= "\n\t" . $attributes_values['products_options_name'] . ' ' . tep_decode_specialchars($order->products[$i]['attributes'][$j]['value']);
// EOF - Zappo - Option Types v2 - Adjustment for Text Option Types
